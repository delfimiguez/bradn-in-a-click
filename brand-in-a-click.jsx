    ...
    >
      ...
      <div className="p-8 flex-1 flex flex-col gap-6 text-sm">
        {tab === "overview" && (
          <>
            <div>
              <p className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest mb-3">Concept</p>
              <p className="text-neutral-300 leading-relaxed">{brand.concept}</p>
            </div>
            <div>
              <p className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest mb-3">Typography</p>
              <div className="space-y-2">
                {Object.entries(brand.typography).map(([role, desc]) => (
                  <div key={role} className="flex gap-3">
                    <span className="text-neutral-600 w-12 flex-shrink-0 capitalize">{role}</span>
        ...
        {tab === "visual" && (
          <>
            <div>
              <p className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest mb-3">Color Logic</p>
              <div className="space-y-2 mb-4">
                {["primary","secondary","accent","background"].map(k => (
                  <ColorSwatch key={k} hex={brand.colorLogic[k]} label={k} />
                ))}
              </div>
              <p className="text-neutral-500 text-xs leading-relaxed">{brand.colorLogic.rationale}</p>
            </div>
            <div>
              <p className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest mb-3">Visual Rules</p>
              <ul className="space-y-2">
                {brand.visualRules.map((r, i) => (
                  <li key={i} className="flex gap-3 text-neutral-400">
                    <span className="text-neutral-700 font-mono">{String(i+1).padStart(2,"0")}</span>
                    {r}
                  </li>
                ))}
              </ul>
        ...
        {tab === "rules" && (
          <div className="grid grid-cols-2 gap-6">
            <div>
              <p className="text-[11px] font-mono text-emerald-600 uppercase tracking-widest mb-3">Do</p>
            ...
            <div>
              <p className="text-[11px] font-mono text-rose-600 uppercase tracking-widest mb-3">Don't</p>
              <ul className="space-y-3">
                {brand.donts.map((d, i) => (
        ...
        {tab === "apply" && (
          <div>
            <p className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest mb-3">Example Applications</p>
            <ul className="space-y-3">
        ...
        >
          ...
          <div className="p-8 space-y-4">
            {[100,80,90,70].map((w,j) => (
              <div key={j} className={`h-3 bg-neutral-800 rounded animate-pulse`} style={{ width: `${w}%`, animationDelay: `${j * 100}ms` }} />
            ))}
          </div>
        </div>
      ))}
  ...
  return (
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <header className="border-b border-neutral-900 px-8 py-5 flex items-center justify-between">
        ...
        <span className="text-[11px] font-mono text-neutral-600 uppercase tracking-widest">Beta</span>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="mb-16 fade-in">
          <h1 className="text-5xl md:text-6xl font-light tracking-tight leading-[1.05] mb-5">
            Three brand directions,<br />
            <em className="text-neutral-500 font-light">instantly.</em>
          </h1>
          <p className="text-neutral-500 text-lg max-w-xl leading-relaxed">
            Describe your brand and get three fully-formed creative directions — concept, color, type, and rules — ready to brief a designer or test with users.
          </p>
        </div>

        {/* Input */}
        <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-6 mb-6 fade-in" style={{ animationDelay: "100ms" }}>
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-transparent text-white placeholder-neutral-700 text-base leading-relaxed resize-none border-none font-normal"
        ...
        {error && (
          <div className="text-rose-400 text-sm mb-8 px-1">{error}</div>
        )}
        ...
        {!loading && brands.length > 0 && (
          <>
            <div className="flex items-center justify-between mb-8 fade-in">
              ...
              <button
                onClick={() => { setBrands([]); setGenerated(false); setInput(""); }}
        ...
        {!loading && !generated && (
          <div className="mt-20 fade-in" style={{ animationDelay: "200ms" }}>
            <p className="text-[11px] font-mono text-neutral-800 uppercase tracking-widest mb-8">Try these prompts</p>
